...
import pickle
from ml_autoencoder import ml_autoencoder as AutoEncoder
import time
import os
...
# file.close()
TrainDatapath = "resources/freepal_large_train_data_type_id"
TestDatapath = "resources/freepal_large_test_data_type_id"
# rel_id_Map = {}
...
# saver.restore(sess, run_path+"/checkpoints/model-26100")
...
save_path = None
ae_model_path = "AE_Models/Model_14_March_ml_sample/model.ckpt"
min_loss = 10
loss = None
...
for i in range(30000):
    print "Epoch : "+str(i)
    _, loss = sess.run([optimizer, auto_encoder.loss], feed_dict= {auto_encoder.input_embeddings: train_data, auto_encoder.dropout : 0.5})
    print "Loss : "+str(loss)
    # if loss >100000:
    #     sess.run(tf.global_variables_initializer())
    #     print "Reinitialized"
...
if(loss<min_loss):
    ...
    print "Model Saved"
    min_loss = loss
print "Saved Model Loss : "+str(min_loss)
