...
p1 = "/home/mvidyasa/Downloads/freepal_relations_with_pattern_greater_than_100"
# rel_id_Map = {}
...
path = p1
x_text_train, y_train, max_document_length = DataExtractor.load_data_and_labels_glove(path)
# max_document_length = max([len(x.split(" ")) for x in x_text_train])
# max_document_length = DataExtractor.g.
...
indomain_count = 0
avg_cost = 0.000173571004391
threshold = 10*avg_cost
...
for i,line in enumerate(lines):
    ...
    x_test = x_train[i]
    x_test = np.reshape(x_test, newshape= [1, 14, 300])
